...
# Compatibility-first template for blast.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/blast" ]; then BIN="/opt/conda/bin/blast"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo blast | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'blast*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/blast
RUN chmod +x /usr/local/bin/blast && rm -f /tmp/tool-entry-path
WORKDIR /data
